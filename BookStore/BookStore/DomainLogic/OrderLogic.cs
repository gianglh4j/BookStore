...
namespace BookStore.DomainLogic
{
    ...
    {
        Task<OrderB> AddOrder(OrderB order, List<DetailDTO> detail);
    ...
    public class OrderLogic : IOrderLogic
    {
        ...
        public async Task<OrderB> AddOrder(OrderB order, List<DetailDTO> details)
        {
            
            var result = await _orderRepository.AddOrder(order);

            //add to OderB_detail
            foreach (DetailDTO detail in details)
            {
                OrderB_Detail order_detail = new OrderB_Detail() { BookId = detail.BookId, OrderId = result.OrderId , Amount = detail.Amount};
